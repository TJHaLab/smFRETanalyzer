...
* SpotTolerance - How deep the dip between two peaks must be before they count as two spots rather than one, in image units on the averaged image. **Leave it at 0** and it is estimated from the image, which is what it is calibrated for. Set it only to override that. This is at the bottom of the dialog rather than beside SpotThreshold because the two are not alternatives: SpotThreshold is the quality bar you choose, while this one is a property of your camera and how many frames you averaged, which is why it is now measured instead of typed in.
